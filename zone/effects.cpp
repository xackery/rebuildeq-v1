...
)
{
	...
	int rank = caster_mob->GetBuildRank(BARD, RB_BRD_SHOUT);
	if (caster_mob && (spell_id == 703 || spell_id == 730 || spell_id == 4806 || spell_id == 1758 ||
		spell_id == 1756 || spell_id == 1764 || spell_id == 746 || spell_id == 736) && rank > 0) {
		caster_mob->BuildEcho(StringFormat("Shout %i increased range from %i to %i", rank, distance, int(distance * 0.2f * rank)));
		distance += floor(distance * 0.2f * rank);
	}
